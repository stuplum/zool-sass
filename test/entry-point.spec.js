...
describe('zool-sass: entryPoint', function () {
    ...
    before(function () {
        temp.create({ 'custom-entry/my-entry.scss': '$test-color: red; body { color: $test-color; }' });
    });
    ...
    beforeEach(function (done) {
        ...
        server.register([{ register: require('../'), options: { src: temp.path, entryPoint: 'my-entry' } }], done);
    });
    ...
    it('should compile from a configured entry point', function (done) {
        ...
        server.inject({ method: 'GET', url: '/css/custom-entry.css' }, function (response) {
            ...
            expect(response.payload).to.be.equal('body{color:red}\n');
            done();
        });
    ...
    it('should write the output to a css file', function (done) {
        ...
        server.inject({ method: 'GET', url: '/css/custom-entry.css' }, function () {
            expect(cssFile('custom-entry.css')).to.be.equal('body{color:red}\n');
            done();
        });
